...
import { axiosGetEntities, getResponseBody, getShortID, timestampToInput } from 'helperFunctions';
import { fieldsOK, getRealDeadline, infoOK, reviewsOK, submissionOK, teamReviewsOK, teamsOK } from './verify';
  ...
  loadForm() {
    ...
    Promise.all([
      ...
      .then(([ questionResponse, materialResponse, fieldResponses ]) => {
        ...
        let allMaterials = getResponseBody(materialResponse).filter((material) => material.URL !== undefined).map((material) => ({
          ...material,
          new: false
        }));
  ...
  render() {
    return (
      <div className="ml-auto center-hor">
        <Button outline color="primary" className="p-1" style={ {width: 34} } onClick={ this.toggle.bind(this) }><i
          className="fa fa-pen"/></Button>
        <Modal isOpen={ this.state.opened } className={ this.props.className }
          ...
          <ModalBody>
              ...
              <NavItem>
                <NavLink
                  className={ classnames({
                    ...
                    hasError: !this.state.formLoaded || !infoOK(this.state.info)
                  }, "clickable") }
                  onClick={ () => {
              ...
              <NavItem>
                <NavLink
                  className={ classnames({
                    ...
                    hasError: !this.state.formLoaded || !submissionOK(this.state.submission)
                  }, "clickable") }
                  onClick={ () => {
              ...
              <NavItem>
                <NavLink
                  className={ classnames({
                    ...
                    hasError: !this.state.formLoaded || !fieldsOK(this.state.fields)
                  }, "clickable") }
                  onClick={ () => {
              ...
              <NavItem>
                <NavLink
                  className={ classnames({
                    ...
                    hasError: !this.state.formLoaded || !teamsOK(this.state.teams)
                  }, "clickable") }
                  onClick={ () => {
              ...
              <NavItem>
                <NavLink
                  className={ classnames({
                    ...
                    hasError: !this.state.formLoaded || !reviewsOK(this.state.reviews, getRealDeadline(this.state.submission))
                  }, "clickable") }
                  onClick={ () => {
                ...
                this.state.formLoaded && !this.state.teams.disabled && <NavItem>
                  <NavLink
                    className={ classnames({
                      ...
                      hasError: !this.state.formLoaded || !teamReviewsOK(this.state.teams, this.state.teamReviews, getRealDeadline(this.state.submission))
                    }, "clickable") }
                    onClick={ () => {
            ...
            { this.state.formLoaded &&
              <TabContent activeTab={ this.state.activeTab }>
                  ...
                  <Submission
                    ...
                    setSubmissionAnonymous={ (anonymousSubmission) => {
                      if(anonymousSubmission && this.state.reviews.visibility === 'open') {
                        this.setState({
                          submission: {...this.state.submission, anonymousSubmission},
                          reviews: {...this.state.reviews, visibility: 'blind'}
                        });
                      } else {
                ...
                <TabPane tabId="3">
                  <Fields data={ this.state.fields } showErrors={ this.state.showErrors } setData={ (fields) => {
                ...
                <TabPane tabId="4">
                  <Teams
                    ...
                    setTeamsDisabled={ (disabled) => {
                      this.setState({
                        teams: {...this.state.teams, disabled},
                        reviews: {...this.state.reviews, reviewedByTeam: !disabled}
                      });
                    } }
                  />
                </TabPane>
          ...
          <ModalFooter>
            ...
            <span>
              ...
              <Button color="primary" disabled={ this.state.activeTab === "1" }
                      onClick={ () => this.setState({activeTab: (parseInt(this.state.activeTab) - 1) + ""}) }>Prev</Button>{ ' ' }
              <Button color="primary" disabled={ this.state.activeTab === "6" } onClick={ () => {
                this.setState({activeTab: (parseInt(this.state.activeTab) + 1) + ""});
              } }>Next</Button>{ ' ' }
              <Button color="success" disabled={ !this.canSave() || this.state.saving }
                      onClick={ this.submitAssignment.bind(this) }>{ this.state.saving ? 'Saving assignment' : 'Save assignment' }</Button>
            </span>
          </ModalFooter>
        </Modal>
      </div>
