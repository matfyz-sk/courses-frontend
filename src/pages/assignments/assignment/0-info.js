      ...
      <div>
        ...
        <div>
          <FormGroup>
            ...
            <Input type="text" id="submission-add-name" value={ this.props.data.name }
                   onChange={ (e) => this.setData('name', e.target.value) }/>
            <ErrorMessage show={ this.props.data.name.length <= 4 && this.props.showErrors }
                          message="Assignment name should be at least 5 characters!"/>
          </FormGroup>
          <FormGroup>
          ...
          <Button size="sm" className="m-2" id="addingExistingMaterialButton" color="primary"
                  onClick={ () => this.setState({addingExistingMaterial: true}) }>
          ...
          <Button size="sm" className="m-2 m-l-10" color="success" id="addingNewMaterialButton"
                  onClick={ () => this.setState({addingNewMaterial: true}) }>
            <i className="fa fa-plus clickable"/>
            Add new material
          </Button>
          <Popover
          ...
          <Table>
            ...
            { this.props.data.hasMaterial.map((material) =>
              <tr key={ material['@id'] }>
                <td>{ material.name }</td>
                <td><a href={ material.URL } target="_blank" without="true"
                       rel="noopener noreferrer">{ material.URL }</a></td>
