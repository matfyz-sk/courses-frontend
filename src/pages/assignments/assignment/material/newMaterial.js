    ...
    return (
      <div>
        <FormGroup>
          ...
          <Input type="text" id="new-material-name" value={ this.state.name }
                 onChange={ (e) => this.setState({name: e.target.value}) }/>
          <ErrorMessage show={ this.state.name.length <= 4 && this.state.showErrors }
                        message="Name should be at least 5 characters!"/>
        </FormGroup>
        <FormGroup>
          <Label for="new-material-url">URL</Label>
          <Input type="url" id="new-material-url" value={ this.state.URL }
                 onChange={ (e) => this.setState({URL: e.target.value}) }/>
          <ErrorMessage show={ this.state.URL.length <= 1 && this.state.showErrors }
                        message="You must set materials address!"/>
        </FormGroup>
