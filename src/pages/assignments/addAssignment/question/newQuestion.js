  ...
  render(){
    return(
      <div>
        <FormGroup check>
          <Label check>
            <Input type="checkbox" checked={this.state.rated} onChange={(e)=>this.setState({ rated: !this.state.rated })} />
            {' '} Rated
          </Label>
        </FormGroup>
        <FormGroup>
          <Label for="new-question-text">Question</Label>
          <Input type="textarea" id="new-question-text" value={this.state.question} onChange={(e)=>this.setState({ question: e.target.value })} />
          <ErrorMessage show={this.state.question.length <= 4 && this.state.showErrors} message="Question should be at least 5 characters!" />
        </FormGroup>
