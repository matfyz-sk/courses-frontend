...
export default class StudentTeamSubmissionsView extends Component {
  ...
  groupByTeam(initial, improved) {
    ...
    initial.forEach((submission) => {
      teams.push({
        ...this.props.teams.find((team) => team['@id'] === submission.submittedByTeam[0]['@id']),
    ...
    return (
      <Table>
        ...
        <tr>
          ...
          { (periodHappening(this.props.assignment.teamReviewPeriod) || periodHasEnded(this.props.assignment.teamReviewPeriod)) &&
            <th></th> }
          <th width="80"></th>
        </tr>
        </thead>
        ...
        { groupedSubmissions.map((team, index) =>
          <tr key={ team['@id'] }>
            ...
            <td className="center-cell">
              { team.initialSubmission ? <i className="fa fa-check green-color"/> :
                <i className="fa fa-times red-color"/> }
            </td>
            { assignment.submissionImprovedSubmission && periodStarted(assignment.improvedSubmissionPeriod) &&
              <td className="center-cell">
                { team.improvedSubmission ? <i className="fa fa-check green-color"/> :
                  <i className="fa fa-times red-color"/> }
              </td>
            }
            { canBeRated &&
            ...
            <td className="center-cell">
              { periodHappening(this.props.assignment.teamReviewPeriod) &&
                <Button color="success"
                        onClick={ () => this.props.history.push(`./assignments/assignment/${ getShortID(this.props.assignment['@id']) }/team/${ getShortID(team['@id']) }/submission/teamReview`) }>Review
                  team</Button>
              }
              { periodHasEnded(this.props.assignment.teamReviewPeriod) &&
                <Button color="success"
                        onClick={ () => this.props.history.push(`./assignments/assignment/${ getShortID(this.props.assignment['@id']) }/team/${ getShortID(team['@id']) }/submission/teamReview`) }>Team
                  results</Button>
              }
            </td>
            <td>
              <Button color={ this.getButtonColor(team.settings) }
                      onClick={ () => this.props.history.push(`./assignments/assignment/${ getShortID(this.props.assignment['@id']) }/team/${ getShortID(team['@id']) }/submission/submission`) }>
