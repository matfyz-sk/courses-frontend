...
import { Alert, Button, Label, Table } from 'reactstrap';
      ...
      <>
        {
          ...
          <div>
            ...
            <Button outline className="ml-2 mb-2 p-1"
                    color={ this.state.initialSubmissions.length === 0 ? 'success' : 'primary' }
                    onClick={ () => this.props.history.push(`./assignments/assignment/${ getShortID(assignment['@id']) }/submission/submission`) }>
        ...
        }
        {
          ...
          <div>
            ...
            <Button outline className="ml-2 mb-2 p-1"
                    color={ this.state.improvedSubmissions.length === 0 ? 'success' : 'primary' }
                    onClick={ () => this.props.history.push(`./assignments/assignment/${ getShortID(assignment['@id']) }/submission/submission`) }>
        ...
        }
        {
          ...
          <div>
            ...
            <Button outline className="ml-2 mb-2 p-1" color="primary"
                    onClick={ () => this.props.history.push(`./assignments/assignment/${ getShortID(assignment['@id']) }/submission/teamReview`) }>Review
              team</Button>
          </div>
        }
        <h5>Your submissions</h5>
        <Alert color="primary" className="row" isOpen={ !this.state.submissionsLoaded || !this.state.reviewsLoaded }>Loading
          submissions...</Alert>
        <Alert color="warning" className="row"
          ...
          <Button className="ml-auto" color="success"
                  onClick={ () => this.props.history.push(`./assignments/assignment/${ getShortID(this.props.assignment['@id']) }/submission/submission`) }>Submit</Button>
        </Alert>
            ...
            <Table>
              ...
              { this.state.toReviews.map((toReview) =>
                <tr key={ toReview['@id'] }>
                  ...
                  <td className="center-cell">{ toReview.review !== undefined ?
                    <i className="fa fa-check green-color"/> : <i className="fa fa-times red-color"/> }</td>
                  <td>
                    <Button
