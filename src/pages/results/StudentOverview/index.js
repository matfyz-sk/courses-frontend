...
import { getAllUserResults, getResultTypeDetail, getUserInCourse, } from '../functions'
...
const StudentOverview = props => {
  ...
  return (
    <Container>
      { user ? (
        <h1
          className="mb-5">{ getUserID() === userId ? 'My results' : `Results of ${ showUserName(user, privileges, courseInstance) }` }</h1>
      ) : null }
      <Row>
