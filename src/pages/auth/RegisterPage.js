...
export default class RegisterPage extends Component {
    ...
    registerValidation() {
        ...
        return errors.first_name.result && errors.last_name.result && errors.email.result && errors.password.result &&
            confirmed_conditions === true && (errors.nickname === null || errors.nickname.result === true);
    }
    ...
    render() {
        const {user, privacy, confirmed_conditions, errors, be_error} = this.state;
        return (
            <React.Fragment>
                ...
                <Row>
                    <Col sm={6} xs={12} key={"col1"}>
                        ...
                        <Form>
                                ...
                                <Col md={6} key={"bi-1"}>
                                    <FormGroup>
                                        ...
                                        <Input type="text" name="first_name" id="first_name" placeholder="Nora"
                                               value={user.first_name} onChange={this.handleInputChange} autoComplete={"first_name"}
                                               valid={textValidator(user.first_name, 3, 20).result}
                                               invalid={errors.first_name ? errors.first_name.result !== true : false}
                                        />
                                ...
                                <Col md={6} key={"bi-2"}>
                                    <FormGroup>
                                        ...
                                        <Input type="text" name="last_name" id="last_name" placeholder="Mojsejova"
                                               value={user.last_name} onChange={this.handleInputChange} autoComplete={"last_name"}
                                               valid={textValidator(user.last_name, 3, 20).result}
                                               invalid={errors.last_name ? errors.last_name.result !== true : false}
                                        />
                                        <FormFeedback tooltip>{errors.last_name ? errors.last_name.msg : ""}</FormFeedback>
                                    </FormGroup>
                                </Col>
                            </Row>
                            <Row form>
                                <Col md={6}>
                                    <FormGroup>
                                        ...
                                        <Input type="email" name="email" id="email" placeholder="nora@example.com"
                                               value={user.email} onChange={this.handleInputChange} autoComplete={"email"}
                                ...
                                <Col md={6}>
                                    <FormGroup>
                                        ...
                                        <Input type="password" name="password" id="password" placeholder="*****************"
                                               value={user.password} onChange={this.handleInputChange} autoComplete={"password"}
                            ...
                            <FormGroup>
                                ...
                                <Input type="textarea" name="about" id="about" placeholder="I am ..." rows={3}
                                       value={user.about} onChange={this.handleInputChange}
                                       valid={textValidator(user.about, 0, 254).result}
                                />
                            </FormGroup>
                            <FormGroup check>
                                <Input type="checkbox" name="confirmed_conditions" id="confirmed_conditions"
                                       checked={confirmed_conditions} onChange={()=>this.setState({confirmed_conditions: !confirmed_conditions})}
                                       invalid={errors.confirmed_conditions !== null ? !errors.confirmed_conditions : null}
                                />
                                <Label for="confirmed_conditions" check>Accept <a href="/privacy-policy" target={"_blank"}>register conditions</a> *</Label>
                                <FormFeedback tooltip>{"You must accept this conditions!"}</FormFeedback>
                            </FormGroup>
                    ...
                    <Col sm={6} xs={12} key={"col2"}>
                        ...
                        <Form>
                            <FormGroup check>
                                <Input type="checkbox" name="use_nickname" id="use_nickname"
                                       onChange={this.handlePrivacyChange} checked={privacy.use_nickname}
                                />
                                <Label for="use_nickname" check>I want to hide my name and use nickname</Label>
                            </FormGroup>
                            <Collapse isOpen={privacy.use_nickname}>
                                <FormGroup>
                                    <Label for="nickname">My nickname will be</Label>
                                    <Input type="text" name="nickname" id="nickname" placeholder="TheBestStudentEver"
                                           onChange={this.handlePrivacyChange} checked={privacy.nickname}
                                           valid={textValidator(privacy.nickname, 5, 20).result}
                                           invalid={errors.nickname ? errors.nickname.result !== true : false}
                                    />
                                    <FormFeedback tooltip>{errors.nickname ? errors.nickname.msg : ""}</FormFeedback>
                                </FormGroup>
                            </Collapse>

                            <FormGroup check>
                                <Input type="checkbox" name="public_profile" id="public_profile"
                                       onChange={this.handlePrivacyChange} checked={privacy.public_profile}
                                />
                                <Label for="public_profile" check>My profile is public</Label>
                            </FormGroup>
                            <Collapse isOpen={!privacy.public_profile}>
                                <FormGroup check>
                                    <Input type="checkbox" name="show_courses" id="show_courses"
                                           onChange={this.handlePrivacyChange} checked={privacy.show_courses}
                                    />
                                    <Label for="show_courses">Show my courses</Label>
                                </FormGroup>
                                <FormGroup check>
                                    <Input type="checkbox" name="show_badges" id="show_badges"
                                           onChange={this.handlePrivacyChange} checked={privacy.show_badges}
                                    />
                                    <Label for="show_badges">Show my badges</Label>
                                </FormGroup>
                                <FormGroup check>
                                    <Input type="checkbox" name="allow_contact" id="allow_contact"
                                           onChange={this.handlePrivacyChange} checked={privacy.allow_contact}
                                    />
