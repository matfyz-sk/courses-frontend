...
function DocumentHistory({
    ...
    useEffect(() => {
        ...
        axiosGetEntities(entitiesUrl).then(response => {
            ...
            if (folder["_id"] !== parentFolderId) fetchFolder(parentFolderId)
    ...
    const handleRestore = async (e, versionToRestore) => {
        ...
        const newVersionId = await editDocument(versionToRestore, latestVersion(), editProps)
                ...
                {(!isMobile || !showSidebar) && (
                    <div style={{ width: isMobile && "100%" }} className={style.versionContentContainer}>
                        ...
                        >
                            ...
                            {status !== 200 && (
                                <Alert color="warning">There has been a server error, try again please!</Alert>
                            )}
                                ...
                                <>
                                    ...
                                    <>
                                        {!hasEmptyContent(pickedVersionB) &&
                                            getPayloadContent(pickedVersionB) !== getPayloadContent(pickedVersionA) && (
                                                <>
                                                    <Link
                                                        id="file-download"
